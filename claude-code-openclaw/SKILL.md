...
description: "Run Claude Code (Anthropic) from this host via the `claude` CLI (Agent SDK) for codebase analysis, refactoring, coding, development, bug fixing, test writing, code review, project building, and complex multi-step tasks. Use when the user asks for coding assistance, code analysis, code review, refactoring, running tests, fixing bugs, implementing features, building projects, writing scripts, or any development work requiring Claude Code CLI."
---
...
```bash
cd /path/to/repo
~/.openclaw/skills/claude-code-clawdbot/scripts/claude_code_run.py \
...
Prefer running them in a managed background session (OpenClaw exec background) or tmux, and explicitly stop them when done.
